...
public class MultipleLockBlockQueueExample2 {
	public static void main(String[] args) {

		
		final BlockQueue2 blockerQueue = new BlockQueue2(3);
		blockerQueue.insert("one");
		blockerQueue.insert("two");
		blockerQueue.insert("three");
		new Thread() {
			public void run() {
				System.out.println("Thread3: attempting to insert an item to the queue");
				blockerQueue.insert("four");
			}
		}.start();
		new Thread() {
			public void run() {
				System.out.println("Thread4: attempting to remove an item from the queue ");
